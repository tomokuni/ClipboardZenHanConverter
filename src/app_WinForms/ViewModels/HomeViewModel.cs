...
public partial class HomeViewModel : ObservableObject, IDisposable
{
    ...
    /// <summary>生成時の UI スレッド同期コンテキスト（WinForms のメッセージループ）。<br/>
    /// テストなど UI スレッド外で生成された場合は null となり、そのまま実行します。</summary>
    ...
    /// <remarks>バインド済みプロパティを更新するため、UI スレッド以外から発火された場合は UI スレッドへ委譲します。</remarks>
    private void Clipboard_ContentChanged(object? sender, object e)
    {
        if (TestMode || _uiContext is null || _uiContext == SynchronizationContext.Current)
        {
            _ = HandleClipboardChangeAsync();
            return;
        }

        _uiContext.Post(static state => _ = ((HomeViewModel)state!).HandleClipboardChangeAsync(), this);
    }
