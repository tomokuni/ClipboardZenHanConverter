<#
.SYNOPSIS
現在のバージョン状況（リポジトリのバージョン・タグ・GitHub Release・配布物）を表示する。
.DESCRIPTION
...
.PARAMETER NoNetwork
GitHub への問い合わせを行わない（オフラインで確認する場合）。
.OUTPUTS
System.String
表示した表（Markdown）。
.EXAMPLE
...
<#
.SYNOPSIS
最新の GitHub Release に添付されている配布物の名前を取得する（取得できない場合は空の配列）。
.PARAMETER Tag
対象のタグ（空の場合は取得しない）。
.OUTPUTS
System.String[]
...
function Get-LatestReleaseAssets {
    ...
    $json = (& gh release view $Tag --json assets 2>$null | Out-String).Trim()
    ...
    try {
        return @(($json | ConvertFrom-Json).assets | ForEach-Object { $_.name })
    }
    catch {
        # 取得できない場合は「なし」として扱う
        return @()
...
# ─── 3. 最新の GitHub Release の添付物 ───
# 配布物が実際に添付されているかを確認できるようにする（配布物の定義は release-config.json が持つ）
$releaseAssets = @()

if (-not $NoNetwork) {
    $releaseAssets = @(Get-LatestReleaseAssets -Tag $latestRelease)
}
